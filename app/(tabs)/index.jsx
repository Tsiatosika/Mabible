...
// ── Cherche un livre par nom (FR ou EN) ou abréviation ──────────────────────
...
function getBookAbrev(bookName) {
  return getBookInfo(bookName)?.abrev || null;
}
...
export default function AccueilScreen() {
  ...
  // Nom du livre de la dernière position selon la langue
  const lastBookInfo = lastPosition ? getBookInfo(lastPosition.book) : null;
  const lastBookName = lastBookInfo
    ? (isFr ? lastBookInfo.nom : lastBookInfo.nomEn)
    : lastPosition?.book || '';
            ...
            >
              <View style={[styles.avatar, { backgroundColor: colors.primary }]}>
                <Text style={styles.avatarText}>
                  {(lastBookInfo?.abrev || lastPosition.book).slice(0, 2).toUpperCase()}
                </Text>
              </View>
              <View style={styles.continueInfo}>
                {/* Nom traduit selon la langue */}
                <Text style={[styles.continueName, { color: colors.textPrimary }]}>
                  {lastBookName}
                </Text>
                <Text style={[styles.continueSub, { color: colors.accent }]}>
                  {isFr ? `Chapitre ${lastPosition.chapter}` : `Chapter ${lastPosition.chapter}`}
                </Text>
              </View>
          ...
          <View style={styles.testamentRow}>
            ...
            >
              <Ionicons name="library-outline" size={24}
                color="rgba(255,255,255,0.9)" style={{ marginBottom: 6 }} />
