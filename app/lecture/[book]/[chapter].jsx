...
export default function LectureScreen() {
  ...
  const { bible, loading, getChapter } = useBible();  // ← bible ajouté ici
  ...
  useEffect(() => {
    if (!bible) return; // Attendre que la Bible soit chargée
  ...
  }, [book, chapter, bible]); // ← bible dans les dépendances
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.errorSub}>Livre : {book} | Chapitre : {chapter}</Text>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {chapData.versets.map((verset) => {
          ...
          return (
            ...
            >
              ...
              <Text style={[styles.versetText, { fontSize }]}>
                {verset.texte}
              </Text>
        ...
        <View style={styles.bottomNav}>
          <TouchableOpacity
            style={[
              styles.bottomBtn,
              chapterNum <= 1 && styles.bottomBtnDisabled,
            ]}
            onPress={() => goChapter(-1)}
            disabled={chapterNum <= 1}
          >
          ...
          <TouchableOpacity
            style={[
              styles.bottomBtn,
              chapterNum >= (bookInfo?.chapitres || 1) && styles.bottomBtnDisabled,
            ]}
            onPress={() => goChapter(1)}
            disabled={chapterNum >= (bookInfo?.chapitres || 1)}
          >
      ...
      {/* Bouton taille de police flottant */}
...
const styles = StyleSheet.create({
  ...
  loadingText:       { color: Colors.textSecondary, fontSize: 16, marginBottom: 8 },
  errorEmoji:        { fontSize: 48, marginBottom: 12 },
  errorSub:          { color: Colors.textLight, fontSize: 13, marginBottom: 16 },
  ...
  chapNav:           { flexDirection: 'row', alignItems: 'center',
                       justifyContent: 'space-between', backgroundColor: Colors.surfaceWarm,
  ...
  versetText:        { flex: 1, color: Colors.textPrimary, lineHeight: 26,
                       fontFamily: 'serif' },
  ...
  modalBox:          { backgroundColor: Colors.surface, borderTopLeftRadius: 20,
                       borderTopRightRadius: 20, padding: 28, paddingBottom: 40 },
  modalTitle:        { color: Colors.textPrimary, fontSize: 17, fontWeight: '700',
                       textAlign: 'center', marginBottom: 20 },
  fontRow:           { flexDirection: 'row', alignItems: 'center',
                       justifyContent: 'center', gap: 24 },
  fontBtn:           { backgroundColor: Colors.surfaceWarm, paddingHorizontal: 22,
                       paddingVertical: 12, borderRadius: 10 },
  fontBtnText:       { color: Colors.primary, fontWeight: '700', fontSize: 18 },
  fontSizeLabel:     { color: Colors.textPrimary, fontSize: 20, fontWeight: '700',
                       minWidth: 55, textAlign: 'center' },

  backBtn:           { marginTop: 20, backgroundColor: Colors.primary,
